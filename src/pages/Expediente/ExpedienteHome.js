...
class ExpedienteHome extends Component {
    render() {
        const getSubTotal = () => {
            let total = 0.00;
            ...
            <Wrapper>
                <Card>
                    <CardContent>
                        ...
                        <Typography variant="body1" className="mb-1"><a href="#!">company@address.com</a></Typography>
                                    ...
                                    <TableRow key={index}>
                                        <CustomTableCell>
                                            <Typography variant="body1"
                                                        className="font-weight-bold">{item.title}</Typography>
                                        ...
                                        <CustomTableCell
                                            numeric>{formatPrice(item.price * item.quantity)}</CustomTableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
                        ...
                        <Grid container spacing={8}>
                            <Grid item xs={6} sm={3} className="mt-xs mb-xs">
                                <Typography variant="caption"
                                            className="font-weight-bold text-uppercase">Subtotal</Typography>
                            ...
                            <Grid item xs={6} sm={3} className="mt-xs mb-xs text-sm-right text-md-left">
                                <Typography variant="caption" className="font-weight-bold text-uppercase">Tax
                                    (15%)</Typography>
                                <Typography variant="headline">{formatPrice(getCalculatedTax())}</Typography>
                            </Grid>
                            <Grid item xs={6} sm={3} className="mt-xs mb-xs">
                                <Typography variant="caption"
                                            className="font-weight-bold text-uppercase">Discount</Typography>
                                <Typography variant="headline">$0.00</Typography>
                            </Grid>
                            <Grid item xs={6} sm={3} className="mt-xs mb-xs text-xs-left text-sm-right">
                                <Typography variant="caption"
                                            className="font-weight-bold text-uppercase">Total</Typography>
                    ...
                    <CardContent>
                        <Typography variant="caption">
                            <strong>PAYMENT TERMS AND POLICIES </strong>. All accounts are to be paid within 7 days from
                            receipt of invoice. To be paid by cheque or credit card or direct payment online. If account
                            is not paid within 7 days the credits details supplied as confirmation of work undertaken
                            will be charged the agreed
                            quoted fee noted above. If the Invoice remails unpaid. our dept recovery agency, Urban, may
                            charge you a fee of 25% of the unpaid portion of the
...
ExpedienteHome.propTypes = {
    classes: PropTypes.object.isRequired,
};

export default ExpedienteHome;
